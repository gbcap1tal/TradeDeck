...
- **Finviz** (via HTML scraping, `server/api/finviz.ts`): Single-pass scraper fetches ALL US-exchange stocks (~5,300) using `ind_stocksonly` filter (excludes ETFs at source) with 600ms page delay, 5 retries with exponential backoff, and 5 consecutive failure threshold. Additionally filters out "Exchange Traded Fund" and "Shell Companies" industries via `EXCLUDED_INDUSTRIES` set. Includes ADRs like BABA, foreign-listed companies on US exchanges. Organizes stocks by native Finviz sector/industry (11 sectors, 144 industries). File-persisted to `.finviz-cache.json` (48h TTL). Provides `getStocksForIndustry()`, `getFinvizSectors()`, and `searchStocks()` for dynamic sector/industry mapping and search. No name translation needed - uses Finviz native classifications directly.
...
- **Market Quality Score** color thresholds: EXCELLENT (90+) #2eb850, GOOD (75-89) #3d8a4e, FAIR (60-74) #2a4a32, NEUTRAL (50-59) #aaaaaa, WEAK (40-49) #6a2a35, POOR (30-39) #b85555, CRITICAL (<30) #d04545.
- **Hourly Scheduler**: ALL data (Finviz, sectors, industry performance, rotation, breadth/Market Quality, megatrend performance) refreshes hourly during market hours: 9:30 AM ET through 4:05 PM ET (5 min after close). Triggers within the first 10 minutes of each hour. Outside market hours and on weekends, data served from cache or file persistence. Initial startup pre-computation runs on server boot.
- **Megatrend Performance**: Multi-timeframe (D/W/M/3M/6M/Y). Daily change from Finviz stock changePercent averages. Weekly/monthly/quarterly/half/yearly computed from Yahoo Finance 1Y price history. Cached in memory (1h TTL) and file-persisted to `.megatrend-perf-cache.json` (24h TTL).
- **IBD RS Rating (1-99 Percentile)**: Python background script (`scripts/compute_rs_ratings.py`) computes true IBD-style Relative Strength ratings. Reads stock universe from Finviz cache (~5,300 tickers), fetches 1Y history via `yfinance` in batches of 100, filters by volume >100k and price >$1, calculates weighted momentum score (0.4×P3 + 0.2×P6 + 0.2×P9 + 0.2×P12), then ranks all ~3,800+ stocks into 1-99 percentile ratings. Output saved to `market_rs_ratings.json`. Server reads this file via `server/api/rs.ts` for O(1) lookups. Supports `--resume` flag for interrupted runs. Run manually or schedule twice weekly. UI color coding: 80-99 green, 50-79 yellow, <50 red.