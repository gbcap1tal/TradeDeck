...
export async function registerRoutes(
  ...
  const BUNDLE_CACHE_TTL = 120;
  const inflightBundles = new Map<string, Promise<any>>();

  function buildBundleData(sym: string, view: string): Promise<any> {
    const existing = inflightBundles.get(sym);
    ...
    const promise = (async () => {
      ...
      const [finvizData, insiderTxns, newsData, quoteData] = await Promise.all([
        (async () => {
          const s = Date.now();
          const r = await scrapeFinvizQuote(sym).catch((e: any) => { console.error(`Bundle finviz error ${sym}:`, e.message); return null; });
          timings.finviz = Date.now() - s;
          return r;
        })(),
        (async () => {
          const s = Date.now();
          const r = await scrapeFinvizInsiderBuying(sym).catch((e: any) => { console.error(`Bundle insider error ${sym}:`, e.message); return [] as any[]; });
          timings.insider = Date.now() - s;
          return r;
        })(),
        (async () => {
          const s = Date.now();
        ...
        (async () => {
          ...
          try {
            ...
            timings.yahooQuote = Date.now() - s;
            console.error(`Bundle quote error ${sym}:`, e.message);
          }
          return null;
        })(),
      ]);
      ...
      if (earnings.length === 0) {
        ...
        timings.yahooEarnings = Date.now() - s;
      }
      if (earnings.length === 0) {
        const s = Date.now();
        try {
        ...
        timings.fmpEarnings = Date.now() - s;
      }
    ...
    promise.finally(() => inflightBundles.delete(sym));
    return promise;
  }
  ...
  app.get('/api/stocks/:symbol/bundle', async (req, res) => {
    ...
    const bundleCacheKey = `stock_bundle_${sym}_${view}`;
    const cached = getCached<any>(bundleCacheKey);
    if (cached) return res.json(cached);

    const stale = getStale<any>(bundleCacheKey);
    if (stale && !isRefreshing(bundleCacheKey)) {
      markRefreshing(bundleCacheKey);
      buildBundleData(sym, view)
        .then(result => setCache(bundleCacheKey, result, BUNDLE_CACHE_TTL))
        .catch((e: any) => console.error(`Background bundle refresh error for ${sym}:`, e.message))
        .finally(() => clearRefreshing(bundleCacheKey));
      return res.json(stale);
    }

    try {
      const result = await buildBundleData(sym, view);
      setCache(bundleCacheKey, result, BUNDLE_CACHE_TTL);
      return res.json(result);
    } catch (e: any) {
      console.error(`Bundle build error for ${sym}:`, e.message);
      if (stale) return res.json(stale);
