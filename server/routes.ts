...
export async function registerRoutes(
  ...
  app.post('/api/leaders/quality', async (req, res) => {
    try {
      ...
      await Promise.allSettled(batch.map(async (sym: string) => {
        const cacheKey = `quality_score_${sym}`;
        ...
        try {
          ...
          if (!snap?.snapshot || Object.keys(snap.snapshot).length === 0) return;
          const s = snap.snapshot;

          const parseP = (v: string | undefined): number => v ? parseFloat(v.replace('%','')) || 0 : 0;
          const parseN = (v: string | undefined): number => {
            if (!v || v === '-') return 0;
            return parseFloat(v.replace(/[,$%]/g,'')) || 0;
          };

          const sma50Pct = parseP(s['SMA50']);
          const sma200Pct = parseP(s['SMA200']);
          const epsQ = parseP(s['EPS Q/Q']);
          const salesQ = parseP(s['Sales Q/Q']);
          const epsTTM = parseN(s['EPS (ttm)']);
          const perfW = parseP(s['Perf Week']);
          const perfM = parseP(s['Perf Month']);
          const relVol = parseN(s['Rel Volume']);
          const operMargin = parseP(s['Oper. Margin']);

          let raw = 0;
          if (sma50Pct > 0) raw += 2;
          if (sma200Pct > 0) raw += 1.5;
          if (sma50Pct > 5) raw += 1.5;
          if (perfW > 0) raw += 1;
          if (perfM > 0) raw += 1;
          if (relVol > 1) raw += 1;
          if (relVol > 1.5) raw += 1;
          ...
          if (epsQ > 25) raw += 1.5;
          if (salesQ > 0) raw += 1;
          if (salesQ > 25) raw += 1.5;
          if (epsTTM > 0) raw += 1.5;
          if (operMargin > 0) raw += 1.5;
          if (operMargin > 15) raw += 1.5;

          const score = Math.min(10, Math.round(raw / 2 * 10) / 10);
          results[sym] = score;
          setCache(cacheKey, score, 3600);
        } catch {}
      }));
